...
class StockTradingEnv(gym.Env):
    def __init__(self, env_config):
        ...
        self.trading_record = pd.DataFrame(columns=["ticker", "date","cash_balance", "share_holding", "asset"
                                        , "transaction_price", "transaction_cost", "trading_outlay", "reward","signal" ])
        # self.trading_record = self.trading_record.append({"ticker": self.ticker
        #                                                 ,"date": self.index_dates[self.index]
        #                                                 ,"cash_balance": self.initial_balance
        #                                                 , "share_holding": 0
        #                                                 , "asset": self.initial_balance
        #                                                 , "transaction_price": random.uniform(self.state[3], self.state[4])
        #                                                 , "transaction_cost": 0
        #                                                 , "trading_outlay": 0
        #                                                 , "reward": 0
        #                                                 ,"signal": 0},ignore_index=True)
        self.balance_record = []
    ...
    def step(self, action):
        ...
        transaction_price = random.uniform(low_price, high_price)

        stock_exchange = 'Frankfurt'
        cost_rate = self._cost_rate(stock_exchange)
        transaction_cost = 0
        reward = 0
        asset = current_balance+transaction_price*current_own_share
        ...
        self.trading_record = self.trading_record.append({"ticker": self.ticker
                                                        ...
                                                        , "share_holding": current_own_share
                                                        , "asset": asset
                                                        , "transaction_price": transaction_price
                                                        , "transaction_cost": transaction_cost
                                                        , "trading_outlay": np.sum(self.trade_cost)
                                                        , "reward": reward
                                                        ,"signal": action},ignore_index=True)
    ...
    def _cost_rate(self, stock_exchange) -> float:
        if stock_exchange == 'Frankfurt':
            return 1.0006
        elif stock_exchange == 'Shanghai':
            return 1.00302
    ...
    def to_excel(self, filename):
        self.trading_record.to_excel(filename)
